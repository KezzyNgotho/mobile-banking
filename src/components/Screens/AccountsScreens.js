...
const AccountsScreen = () => {
  ...
  useEffect(() => {
    ...
    const handleTransfer = async () => {
        ...
        // Your existing transfer logic...
          ...
          try {
            if (!selectedBank || !recipientAccount || !amount) {
              Alert.alert('Error', 'Please select a bank, enter recipient\'s account number, and enter the amount.');
              return;
            }
            ...
            if (checkingBalance < transferAmount) {
              Alert.alert('Error', 'Insufficient funds. Please check your balance.');
              return;
            }
        ...
        const checkingDocRef = firestore().collection('totals').doc('checking');
        ...
        await firestore().runTransaction(async (transaction) => {
          ...
          if (checkingData) {
            const updatedChecking = {
              amount: checkingData.amount - parseFloat(amount),
   ...
   await firestore().collection('transactions').add({
    ...
    type: 'Transfer', // Type can be 'Transfer', 'Deposit', 'Withdrawal', etc.
    bank: selectedBank,
